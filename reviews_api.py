...
# __file__ always finds the correct location,if stored in another folder
...
def get_product_rating(product_id: int) -> dict:
    ...
    cursor.execute(
        ...
        (product_id,),
    )#Only those rows whose product_id matches the value
    row = cursor.fetchone()
    conn.close()
...
def get_ratings_for_products(product_ids: list[int]) -> list[dict]:#list of integers expected
    """Return ratings for a list of product IDs."""
    if not product_ids:
        return []
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    placeholders = ",".join("?" * len(product_ids))#It joins everything using a comma
    cursor.execute(
        f"""
        ...
        """,
        product_ids,
    )#no.of product IDs changes for every request,Dynamic placeholders allow one SQL query to safely handle any number of IDs
...
#If key doesn't exist,return {} instead.
